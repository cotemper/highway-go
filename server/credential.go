...
func (ws *Server) RequestNewCredential(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range takenNames {
		if strings.ToLower(x) == strings.ToLower(username) {
			available = false
			break
		}
	}
